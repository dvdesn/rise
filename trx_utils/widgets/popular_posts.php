 ...
 */
add_action( 'widgets_init', 'grace_church_widget_popular_posts_load' );

/**
 * Register our widget.
 */
function grace_church_widget_popular_posts_load() {
	register_widget('grace_church_widget_popular_posts');
}

/**
 * Most popular and commented Widget class.
 */
class grace_church_widget_popular_posts extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'grace_church_widget_popular_posts');

		/* Create the widget. */
		parent::__construct( 'grace_church_widget_popular_posts', esc_html__('Grace-Church - Most Popular & Commented Posts', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? grace_church_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($q->have_posts()) {
				...
				while ($q->have_posts()) { $q->the_post();
					$post_number++;
					require(grace_church_get_file_dir('templates/_parts/widgets-posts-info.php'));
		...
		if (count($tabs) > 0) {
			...
			echo grace_church_do_shortcode('[trx_tabs style="2"]'.join($tabs).'[/trx_tabs]');
