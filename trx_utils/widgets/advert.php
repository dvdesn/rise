 ...
 */
add_action( 'widgets_init', 'grace_church_widget_advert_load' );

/**
 * Register our widget.
 */
function grace_church_widget_advert_load() {
	register_widget( 'grace_church_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class grace_church_widget_advert extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'grace_church_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'grace_church_widget_advert', esc_html__('Grace-Church - Advertisement block', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(grace_church_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			grace_church_show_layout(grace_church_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
