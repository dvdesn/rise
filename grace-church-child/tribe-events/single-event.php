...
<div id="tribe-events-content" class="tribe-events-single vevent hentry">
	...
	<p class="tribe-events-back">
		<a href="<?php echo esc_url( tribe_get_events_link() ); ?>"> <?php printf( esc_html__( '&laquo; All %s', 'grace-church' ), $events_label_plural ); ?></a>
	</p>
	...
	<div id="tribe-events-header" <?php tribe_events_the_header_attributes() ?>>
		<!-- Navigation -->
		<h3 class="tribe-events-visuallyhidden"><?php printf( esc_html__( '%s Navigation', 'grace-church' ), $events_label_singular ); ?></h3>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<div class="tribe-events-schedule updated published tribe-clearfix">
                ...
                if( !empty($phone) || !empty($email) || !empty($website) || !empty($organizer) ) {
                    ...
                    [trx_button style="filled" size="small" link="#popupform_single_event" popup="on" ]Contáctanos[/trx_button]
                    [trx_popup id="popupform_single_event"]'
                        . '<div class="popup_info title"><h4>' . esc_html__('Organizador', 'grace-church') . '</h4></div>'
                        . (!empty($organizer)
                            ? '<div class="popup_info organizer">' . '<span>' . esc_html__('Título: ', 'grace-church') . '</span>' . ($organizer) . '</div>'
                            : '')
                        . (!empty($phone)
                            ? '<div class="popup_info phone">' . '<span>' . esc_html__('Phone: ', 'grace-church') . '</span><a href="tel:' . ($phone) . '">' . ($phone) . '</a></div>'
                            : '')
                        . (!empty($email)
                            ? '<div class="popup_info email">' . '<span>' . esc_html__('Email: ', 'grace-church') . '</span><a href="mailto:' . ($email) . '">' . ($email) . '</a></div>'
                            : '')
                        . (!empty($website)
			...
			<div class="tribe-events-single-event-description tribe-events-content entry-content description">
				<div class="description-title"><?php echo esc_html__('Descripción', 'grace-church'); ?></div>
                <?php the_content(); ?>
			</div>
