...
if (!function_exists('grace_church_show_breadcrumbs')) {
	function grace_church_show_breadcrumbs($args=array()) {
		...
		$args = array_merge( array(
			'home' => esc_html__('Inicio', 'grace-church'),		// Home page title (if empty - not showed)
...
if ( !function_exists( 'grace_church_callback_ajax_search' ) ) {
	function grace_church_callback_ajax_search() {
		...
		if (!empty($s)) {
			...
			if (empty($output)) {
				$output .= '<article class="post_item">' . esc_html__('Lo sentimos, pero nada coincide con sus criterios de búsqueda. Intente nuevamente con algunas palabras clave diferentes.', 'grace-church') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more search_more">' . esc_html__('Más resultados &hellip;', 'grace-church') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = esc_html__('¡La cadena de consulta está vacía!', 'grace-church');
		}
...
// AJAX: Set post likes/views count
// Handler of add_action('wp_ajax_post_counter', 			'grace_church_callback_post_counter');
// Handler of add_action('wp_ajax_nopriv_post_counter',	'grace_church_callback_post_counter');
