...
if ( !function_exists( 'grace_church_template_services_1_theme_setup' ) ) {
	add_action( 'grace_church_action_before_init_theme', 'grace_church_template_services_1_theme_setup', 1 );
	function grace_church_template_services_1_theme_setup() {
		grace_church_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'grace-church'),
...
// Template output
if ( !function_exists( 'grace_church_template_services_1_output' ) ) {
	function grace_church_template_services_1_output($post_options, $post_data) {
		$show_title = true;
			...
			<div<?php grace_church_show_layout( $post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					...
					if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
						?><a class="sc_services_icon" href="<?php echo esc_url($post_data['post_link']); ?>"><?php grace_church_show_layout($html); ?></a><?php
					} else
						grace_church_show_layout($html);
				} else {
					?>
                ...
                if ($show_title) {
                    if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
                        ?><h4 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php grace_church_show_layout( $post_data['post_title']); ?></a></h4><?php
                    } else {
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !grace_church_param_is_off($post_options['readmore'])) {
                                echo do_shortcode('[trx_button style="filled" link="' . esc_url($post_data['post_link']) . '" class=" sc_services_item_readmore_1 " top="2rem" bg_style="custom"]' . trim($post_options['readmore']) . '[/trx_button]');
                            }
						}
						?>
					</div>
