...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = grace_church_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php grace_church_show_layout( $page_style=='single-portfolio-fullscreen' ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''); ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') grace_church_show_layout( $post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			$args = grace_church_query_add_posts_and_cats($args, '', $post_data['post_type'],
			...
			if ( $prev ) {
				...
				$desc = grace_church_strshort($prev->post_title, 30);
				?>
				<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
					<span class="post_nav_info">
						...
						<span class="post_nav_info_description"><?php grace_church_show_layout( $desc); ?></span>
					</span>
				</a>
				<?php
			}
			if ( $next ) {
				...
				$desc = grace_church_strshort($next->post_title, 30);
				?>
