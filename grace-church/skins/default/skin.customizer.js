...
function grace_church_skin_customizer(option, val) {
    "use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = GRACE_CHURCH_GLOBALS['co_add_params']!=undefined ? GRACE_CHURCH_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#grace_church-customizer-styles-'+option).length > 0 ? jQuery('#grace_church-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="grace_church-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
