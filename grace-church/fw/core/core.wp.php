...
if (!function_exists('grace_church_show_breadcrumbs')) {
	function grace_church_show_breadcrumbs($args=array()) {
		...
		if ($args['echo'] && !empty($rez)) grace_church_show_layout( $rez);
...
if (!function_exists('grace_church_show_pagination_single')) {
	function grace_church_show_pagination_single( $args ) {
		...
		if ( $wp_query->max_num_pages < 2 && ( is_home() || is_archive() || is_search() ) )
			return;
		grace_church_show_layout( $args['before']);
		if ( is_single() ) {
			?>
		...
		grace_church_show_layout( $args['after']);
	}
}
...
if (!function_exists('grace_church_show_pagination_blog')) {
	function grace_church_show_pagination_blog($opt) {
		...
		grace_church_show_layout( $output);
	}
}
...
if ( !function_exists( 'grace_church_callback_view_more_posts' ) ) {
	function grace_church_callback_view_more_posts() {
		...
		$page = grace_church_get_value_gp('page');
...
if ( !function_exists( 'grace_church_callback_ajax_search' ) ) {
	function grace_church_callback_ajax_search() {
		...
		$s = grace_church_get_value_gp('text');
...
if (!function_exists('grace_church_get_calendar')) {
	function grace_church_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if ( isset($_GET['w']) ) $w = ''.intval(grace_church_get_value_gp('w'));
		...
		$calendar_caption = esc_html_x('%1$s %2$s', 'calendar caption', 'grace-church');
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) || !empty($posts['0' . $day]) ) {
				$posts[$day] = !empty($posts[$day]) ? $posts[$day] : $posts['0' . $day];
...
if ( !function_exists( 'grace_church_callback_calendar_change_month' ) ) {
	function grace_church_callback_calendar_change_month() {
		...
		$m = (int) grace_church_get_value_gp('month');
		$y = (int) grace_church_get_value_gp('year');
		$pt = grace_church_get_value_gp('post_type');
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							<span class="comment_author"><?php grace_church_show_layout(( $author_id ? '<a href="'.esc_url($author_link).'">' : '') . comment_author() . ($author_id ? '</a>' : '')); ?></span>
...
if ( !function_exists( 'grace_church_show_taxonomies_filters' ) ) {
	function grace_church_show_taxonomies_filters() {
		...
		if ($page == 'post')
			...
			return;
		grace_church_show_layout(grace_church_get_terms_filters($taxes));
	}
}
...
if (!function_exists('grace_church_get_post_data')) {
	function grace_church_get_post_data(&$opt, $post_obj=null) {
		...
		if ($opt['reviews'] && function_exists('grace_church_reviews_theme_setup')) {
		...
		if ($opt['content']) {
			if (!empty($post_content) && $opt['substitute_gallery'] && !grace_church_in_shortcode_blogger(true))	$post_content = grace_church_substitute_gallery($post_content, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop'], true);
		...
		if ($post_format == 'gallery') {
			$post_gallery = grace_church_build_gallery_tag(grace_church_get_post_gallery($post_content_plain, $post_id, max(2, grace_church_get_custom_option('gallery_max_slides'))), $thumb_sizes['w'], $thumb_sizes['h_crop'], false, $post_link);
			...
			if ($post_video!='' && $opt['substitute_video']) {
				$post_video = grace_church_substitute_video($post_video, $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);
			}
...
if ( !function_exists( 'grace_church_query_posts_where' ) ) {
	// Handler of add_filter('posts_where', 'grace_church_query_posts_where', 10, 2);
...
if ( !function_exists( 'grace_church_query_add_posts_per_page' ) ) {
	// Handler of add_action( 'pre_get_posts', 'grace_church_query_add_posts_per_page' );
...
// Filter categories list - exclude unwanted cats from widget output
// Handler of add_action( 'widget_categories_args', 			'grace_church_query_exclude_categories' );
// Handler of add_action( 'widget_categories_dropdown_args',	'grace_church_query_exclude_categories' );
...
//Handler of add_action( 'widget_posts_args', 'grace_church_query_exclude_posts' );
