...
if (!function_exists('grace_church_post_type_theme_setup')) {
	...
	function grace_church_post_type_theme_setup() {
		...
		if ($override_key) {
            ...
            add_filter('trx_utils_filter_override_options',			'grace_church_post_type_add_override_options');
			add_action('admin_enqueue_scripts', 'grace_church_post_type_admin_scripts');
			// Create override options
			global $GRACE_CHURCH_GLOBALS;
			$GRACE_CHURCH_GLOBALS['post_override_options'] = array(
...
if (!function_exists('grace_church_post_type_admin_scripts')) {
	//Handler of add_action('admin_enqueue_scripts', 'grace_church_post_type_admin_scripts');
...
if (!function_exists('grace_church_post_type_add_override_options')) {
	//Handler of add_action('admin_menu', 'grace_church_post_type_add_override_options');
	function grace_church_post_type_add_override_options($boxes = array()) {
        $boxes[] = array_merge(grace_church_get_global('post_override_options'), array('callback' => 'grace_church_post_type_show_override_options'));
        return $boxes;
	}
}

// Callback function to show fields in override options
if (!function_exists('grace_church_post_type_show_override_options')) {
	function grace_church_post_type_show_override_options() {
        ...
        echo '<input type="hidden" name="override_options_post_nonce" value="' .esc_attr($GRACE_CHURCH_GLOBALS['admin_nonce']).'" />';
		...
		$mb = $GRACE_CHURCH_GLOBALS['post_override_options'];
		$post_options = grace_church_array_merge($GRACE_CHURCH_GLOBALS['options'], $mb['fields']);

		do_action('grace_church_action_post_before_show_override_options', $post_type, $post->ID);
		...
		do_action('grace_church_action_post_after_show_override_options', $post_type, $post->ID);
...
if (!function_exists('grace_church_post_type_save_options')) {
	//Handler of add_action('save_post', 'grace_church_post_type_save_options');
	function grace_church_post_type_save_options($post_id) {
        global $GRACE_CHURCH_GLOBALS;
		// verify nonce
        if (!isset($_POST['override_options_post_nonce']) || !wp_verify_nonce($_POST['override_options_post_nonce'], $GRACE_CHURCH_GLOBALS['admin_url'])) {
			return $post_id;
		}
		...
		$post_type = isset($_POST['override_options_post_type']) ? grace_church_get_value_gp('override_options_post_type') : grace_church_get_value_gp('post_type');
		$override_key = grace_church_get_override_key($post_type, 'post_type');
		...
		$post_options = array_merge($GRACE_CHURCH_GLOBALS['options'], $GRACE_CHURCH_GLOBALS['post_override_options']['fields']);
